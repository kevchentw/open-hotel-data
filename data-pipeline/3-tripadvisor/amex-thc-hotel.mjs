...
const SOURCE = "amex_thc";
const PLAN_LABEL = "THC";
const PLAN_NAME = "The Hotel Collection";
...
const INPUT_STAGE_ONE_FILE_URL = new URL("../1-list/amex-thc-hotel.json", import.meta.url);
const OUTPUT_FILE_URL = new URL("./amex-thc-hotel.json", import.meta.url);
const OUTPUT_DIRECTORY_URL = new URL("./", import.meta.url);

export async function buildAmexThcTripadvisorMatches() {
  ...
  if (pendingHotels.length === 0) {
    console.log(`${PLAN_LABEL} TripAdvisor backfill is up to date. ${totalHotels}/${totalHotels} hotels already have TripAdvisor data.`);
  ...
  console.log(
    `Starting ${PLAN_LABEL} TripAdvisor backfill. ${existingCount}/${totalHotels} already matched, ${pendingHotels.length} pending.`
  );
  ...
  try {
    ...
    await browserSession.waitForManualReady(
      `Brave Search is open in a headed browser. Complete any cookie or bot checks there, then press Enter here to start ${PLAN_NAME} backfill.`
    );
...
export async function writeStageThreeOutputs() {
  const payload = await buildAmexThcTripadvisorMatches();

  console.log(
    `Wrote ${payload.metadata.record_count} ${PLAN_LABEL} stage 3 records to ${OUTPUT_FILE_URL.pathname} ` +
