...
const INPUT_STAGE_ONE_FILE_URL = new URL("../1-list/bilt-hafh-hotel.json", import.meta.url);
const OUTPUT_FILE_URL = new URL("./bilt-hafh-hotel.json", import.meta.url);
const OUTPUT_DIRECTORY_URL = new URL("./", import.meta.url);

export async function buildBiltHafhTripadvisorMatches() {
  ...
  if (pendingHotels.length === 0) {
    console.log(`Bilt TripAdvisor backfill is up to date. ${totalHotels}/${totalHotels} hotels already have TripAdvisor data.`);
  ...
  console.log(
    `Starting Bilt TripAdvisor backfill. ${existingCount}/${totalHotels} already matched, ${pendingHotels.length} pending.`
  );
  ...
  try {
    ...
    await browserSession.waitForManualReady(
      "Brave Search is open in a headed browser. Complete any cookie or bot checks there, then press Enter here to start Bilt backfill."
    );
...
export async function writeStageThreeOutputs() {
  const payload = await buildBiltHafhTripadvisorMatches();

  console.log(
    `Wrote ${payload.metadata.record_count} Bilt stage 3 records to ${OUTPUT_FILE_URL.pathname} ` +
